...
SAVE_DIR = '.'  # images saved to repo root
...
def save_confusions(y_true, y_pred):
    ...
    plt.tight_layout(); plt.savefig(os.path.join(SAVE_DIR, 'confusion_counts.png'), dpi=160); plt.close()
    ...
    plt.tight_layout(); plt.savefig(os.path.join(SAVE_DIR, 'confusion_normalized.png'), dpi=160); plt.close()
...
def save_roc(y_true, y_prob):
    ...
    plt.savefig(os.path.join(SAVE_DIR, 'roc_curve.png'), dpi=160); plt.close()
...
def save_pr(y_true, y_prob):
    ...
    plt.legend(); plt.tight_layout()
    plt.savefig(os.path.join(SAVE_DIR, 'precision_recall_curve.png'), dpi=160); plt.close()

def save_threshold_sweep(y_true, y_prob):
    # Utility plot: how precision/recall/accuracy change with threshold
    ...
    plt.xlabel('Decision Threshold (Fall probability)')
    plt.ylabel('Score')
    plt.title('Threshold vs Precision/Recall/Accuracy – Test')
    plt.legend(); plt.tight_layout()
    plt.savefig(os.path.join(SAVE_DIR, 'threshold_sweep.png'), dpi=160); plt.close()

def main():
    y_true = np.load('data/processed/y_test.npy')
    y_pred = np.load('data/processed/y_pred.npy')
    y_prob = np.load('data/processed/y_prob.npy')
